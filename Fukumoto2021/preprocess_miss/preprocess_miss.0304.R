...
# data.miss <- data.miss[data.miss$prior.infection.0304 == 0, ]

# Remove 62 municipalities in Tokyo and elsewhere missing COVID data
#data.miss <- data.miss[is.na(data.miss$X2020.3.4) == FALSE,]

# Calculate infections in the last week and prior infection per capita
data.miss$cumulative.week.0304 <- rowSums(data.miss[,84:90])/data.miss$pop.2020
data.miss$prior.infection.0304 <- rowSums(data.miss[,52:90], na.rm=T)
data.miss$prior.infection.per.capita <- data.miss$prior.infection.0304/data.miss$pop.2020
#data.miss <- data.miss[is.na(data.miss$cumulative.week.0304) == FALSE,]
...
# Set electoral time
data.miss$date.0304 <- as.character(data.miss$date.0304)
data.miss$electoral.time <- as.numeric(as.Date("2020-03-04") - as.Date(data.miss$date.0304, "%m/%d/%y"))

### Added Items ###

# Treatment indicator
treat <- data.miss$shutdown.0304
...
# Previous Cases
data.miss$cases_1 <- data.miss$X2020.3.3 / data.miss$pop.2020
data.miss$cases_2 <- data.miss$X2020.3.2 / data.miss$pop.2020
data.miss$cases_3 <- data.miss$X2020.3.1 / data.miss$pop.2020
data.miss$cases_4 <- data.miss$X2020.2.29 / data.miss$pop.2020
data.miss$cases_5 <- data.miss$X2020.2.28 / data.miss$pop.2020
data.miss$cases_6 <- data.miss$X2020.2.27 / data.miss$pop.2020
data.miss$cases_7 <- data.miss$X2020.2.26 / data.miss$pop.2020
