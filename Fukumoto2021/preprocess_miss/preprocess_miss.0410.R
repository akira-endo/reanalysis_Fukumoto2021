...
# data.miss <- data.miss[is.na(data.miss$shutdown.0410) == FALSE,]
# data.miss <- data.miss[is.na(data.miss$shutdown.0406) == FALSE,]
...
# Don't remove municipalities with prior infections
# data.miss <- data.miss[data.miss$prior.infection.0410 == 0, ]

# Calculate infections in the last week and prior infection per capita
data.miss$cumulative.week.0410 <- rowSums(data.miss[,121:127])/data.miss$pop.2020
data.miss$prior.infection.0410 <- rowSums(data.miss[,c(52:127, 284)], na.rm=TRUE)
data.miss$prior.infection.per.capita <- data.miss$prior.infection.0410/data.miss$pop.2020
#data.miss <- data.miss[is.na(data.miss$cumulative.week.0410) == FALSE,]
...
# Set electoral time
data.miss$date.0410 <- as.character(data.miss$date.0410)
data.miss$electoral.time <- as.numeric(as.Date("2020-04-10") - as.Date(data.miss$date.0410, "%m/%d/%y"))

### Added Items ###

# Treatment indicator
treat <- data.miss$shutdown.0410
...
# Previous Cases
data.miss$cases_1 <- data.miss$X2020.4.9 / data.miss$pop.2020
data.miss$cases_2 <- data.miss$X2020.4.8 / data.miss$pop.2020
data.miss$cases_3 <- data.miss$X2020.4.7 / data.miss$pop.2020
data.miss$cases_4 <- data.miss$X2020.4.6 / data.miss$pop.2020
data.miss$cases_5 <- data.miss$X2020.4.5 / data.miss$pop.2020
data.miss$cases_6 <- data.miss$X2020.4.4 / data.miss$pop.2020
data.miss$cases_7 <- data.miss$X2020.4.3 / data.miss$pop.2020