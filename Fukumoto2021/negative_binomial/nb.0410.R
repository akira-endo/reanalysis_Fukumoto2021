...
# estimate the ATT for every day from one week before (April 3) to three weeks after (May 1)
part <- 124:152 # indicates April 3 to May 1
part.adj <- part - 54

summary.NB.out <- matrix(NA, length(part), 2)
for(p in 1:length(part)){
  fit <- glm.nb(gm[, part[p]] ~ shutdown.0410 + offset(log(pop.2020)),
