...
data.analysis <- data.analysis %>%
  mutate(cumulative.week.0316 = rowSums(data.analysis[,96:102])/pop.2020,
         prior.infection.0316 = rowSums(data.analysis[,52:102], na.rm=T),
         prior.infection.per.capita = prior.infection.0316/pop.2020,
         neighbor.infection = as.numeric(as.character(outcome.neighbor(cumulative.week.0316, neighbor.matrix.std, pop.2020))),
         neighbor.treatment = as.numeric(as.character(shutdown.neighbor(shutdown.0316, neighbor.matrix.std))),
         date.0316 = as.character(date.0316),
         electoral.time = as.numeric(as.Date("2020-03-16") - as.Date(data.analysis$date.0316, "%m/%d/%y"))) %>%
  filter(is.na(neighbor.treatment) == FALSE & # only municipalities where neighbor.treatment = 1
           ...
           pref %in% c(3, 6, 7, 9, 11, 12, 13, 15, # limit to small size dataset
                       ...
                       41, 42, 44, 45, 46) &
           is.na(shutdown.0316) == FALSE & # remove missing data on shutdown status
           is.na(shutdown.0304) == FALSE &
           is.na(tmean_mean) == FALSE & # sunshine, precipitation also missing for same municipality
           is.na(X2020.3.16) == FALSE & # removes missing data on COVID (i.e., Tokyo)
           is.na(cumulative.week.0316) == FALSE & # removes missing data on cumulative week
...
# Treatment indicator
treat <- data.analysis$shutdown.0316
...
data.analysis <- data.analysis %>%
  mutate(log.number = log(1 + data.analysis$number.of.neighbors),
         cases_1 = X2020.3.15 / pop.2020,
         cases_2 = X2020.3.14 / pop.2020,
         cases_3 = X2020.3.13 / pop.2020,
         cases_4 = X2020.3.12 / pop.2020,
         cases_5 = X2020.3.11 / pop.2020,
         cases_6 = X2020.3.10 / pop.2020,
         cases_7 = X2020.3.9 / pop.2020)
