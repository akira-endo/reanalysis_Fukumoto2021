...
data.analysis <- data.analysis %>%
  mutate(cumulative.week.0406 = rowSums(data.analysis[,117:123])/pop.2020,
         ...
         neighbor.infection = as.numeric(as.character(outcome.neighbor(cumulative.week.0406, neighbor.matrix.std, pop.2020))),
         neighbor.treatment = as.numeric(as.character(shutdown.neighbor(shutdown.0406, neighbor.matrix.std))),
