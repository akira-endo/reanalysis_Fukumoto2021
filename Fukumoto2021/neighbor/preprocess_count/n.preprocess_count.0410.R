...
data.analysis <- data.analysis %>%
  mutate(cumulative.week.0410 = rowSums(data.analysis[,121:127])/pop.2020,
         ...
         neighbor.infection = as.numeric(as.character(outcome.neighbor(cumulative.week.0410, neighbor.matrix.std, pop.2020))),
         neighbor.treatment = as.numeric(as.character(shutdown.neighbor(shutdown.0410, neighbor.matrix.std))),
  ...
  filter(is.na(neighbor.treatment) == FALSE & # only municipalities where neighbor.treatment = 1
           ...
           is.na(X2020.4.10) == FALSE & # removes missing data on COVID (i.e., Tokyo)
