...
# 
# # Save Weights
# saveRDS(W.out, "ipw/results/w.out_0304.RDS")
...
# Load Weights
W.out <- readRDS("ipw/results/w.out_0304.RDS")
...
# estimate the ATC for one week before (Feb. 26) to three weeks after (March 25)
part <- 84:112 # indicates Feb.26 to March 25
part.adj <- part - 51 
...
models <- lapply(varlist, function(x){
  svyglm(substitute(i ~ shutdown.0304, list(i = as.name(x))), design = d.w)
})
